...
class Shaper(object):
        ...
        for node_ips in shaping_conf:
            ...
            for ip in node_ips:
                ...
                if ip in self.ip_handles:
                    ...
                    continue
                logger.info("Configuring {0} -> U:{1}{units} D:{2}{units} delay:{3}ms"\
                            .format(ip, upload, download, delay, units=settings.UNITS))
                # upload shaping for the given set of IPs
                self.__shape_upload(ip, upload, delay)                    
                # download shaping
    ...
    def __shape_upload(self, ip, rate, delay):
    ...
    def __shape_download(self, ip, rate, delay):
        ...
        self.__shape_ifb(self.ifb_down, 'dst %s' % ip, qh, rate, nh, delay)
    
    
    def __shape_ifb(self, ifb, ip, qhandle, rate, nhandle, delay):
